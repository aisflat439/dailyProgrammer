...
function translateRange(inputRange){
  ...
  inputRange.map((item, index, array) => {
    if (typeof translatedRange === undefined || translatedRange.length === 0) {
      translatedRange.push(item);
      return;
    }
    if (item > translatedRange[translatedRange.length - 1]) {
      translatedRange.push(item);
    } else {
      item += 10;
      // while (item > translatedRange[translatedRange.length - 1]) {
      // }
      translatedRange.push(item);
    }
  });
